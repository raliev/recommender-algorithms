...
class CMLVisualizer(AlgorithmVisualizer):
    ...
    def _plot_recommendation_breakdown(self):
        """
        Plots the recommendation breakdown for a single sample user using negative distance scores.
        """
        ...
        # Calculate negative distance scores for this user
        result_vector = -np.linalg.norm(self.P[sample_user_idx, np.newaxis, :] - self.Q, axis=1)
