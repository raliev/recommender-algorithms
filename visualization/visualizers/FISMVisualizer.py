# visualization/visualizers/FISMVisualizer.py
import os
import json
import numpy as np
import pandas as pd
...
class FISMVisualizer(AlgorithmVisualizer):
    ...
    def start_run(self, params, R=None):
        ...
        self.R = R # Store the training matrix (R_binary)
    ...
    def _plot_convergence_graphs(self):
        """Plots FISM convergence (factor changes only)."""
    ...
    def _plot_recommendation_breakdown(self):
        ...
        Uses a P @ Q.T approximation for visualization, even if the
        model's predict() is different.
        """
        ...
        # 3. Calculate full prediction scores (R_u @ (P @ Q.T))
        # FISM's prediction is R @ (P @ Q.T)
        # The breakdown plotter expects (user_history * similarity_matrix)
        # So, the similarity matrix is P @ Q.T

        similarity_matrix = self.P @ self.Q.T
        result_vector = user_history_vector @ similarity_matrix
    ...
    def end_run(self):
        ...
        self._save_history()
        self._save_visuals_manifest() # Save the *complete* manifest